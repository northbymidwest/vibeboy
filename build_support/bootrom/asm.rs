...
impl Asm {
    ...
    pub fn ld_bc(&mut self, nn: u16) {
        self.emit(&[0x01, nn as u8, (nn >> 8) as u8]);
    }
    pub fn ld_sp(&mut self, nn: u16) {
        self.emit(&[0x31, nn as u8, (nn >> 8) as u8]);
    }
    ...
    pub fn ldh_n_a(&mut self, n: u8) {
        self.emit(&[0xE0, n]);
    }
    pub fn ldh_a_n(&mut self, n: u8) {
        self.emit(&[0xF0, n]);
    }
    ...
    pub fn ld_a_nn(&mut self, nn: u16) {
        self.emit(&[0xFA, nn as u8, (nn >> 8) as u8]);
    }
    pub fn ld_ffc_a(&mut self) {
        self.emit(&[0xE2]);
    }
