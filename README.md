...
34 scaling filters across 20 algorithm modules, each available both on the CPU and as a GPU compute shader:
...
The vectorize filter converts each frame to smooth vector paths using the [Kopf-Lischinski algorithm](https://johanneskopf.de/publications/pixelart/) via an 8-pass GPU pipeline (similarity graph, crossing resolution, cell graph, energy optimization, T-junction and crossing update, B-spline rasterization). Falls back to a CPU implementation of the same stages when no GPU is available. Runs in real-time.
...
- **Scaling**: 34 scaling filters across 20 algorithm modules, all with GPU compute shader acceleration
- **Vectorization**: Kopf-Lischinski pixel-art vectorizer with an 8-pass GPU pipeline (CPU fallback); SVG export
...
# Run every test suite and compare per test against tests/accuracy-baseline.txt
./scripts/accuracy.sh
```
...
├── snapshot.rs      Emulator state snapshots (save states, rewind)
├── savestate.rs     Versioned save state encoding
├── rewind.rs        Reverse-delta rewind buffer
...
├── ui_util.rs       Shared native frontend Session (emulation tick, saves, audio ring, gamepad)
├── cpal_audio.rs    cpal audio output (winit, GTK)
...
The core emulator is a pure computation engine with no I/O, filesystem, or platform dependencies. Time is injected via the `Clock` trait. Frontends handle rendering, audio output, input, and persistence; the native ones share `ui_util::Session` for everything else.
