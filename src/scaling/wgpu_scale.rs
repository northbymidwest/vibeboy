...
impl WgpuScaleFilter {
    ...
    pub fn from_scale_filter(f: super::ScaleFilter) -> Option<Self> {
        ...
        match f {
            ...
            SF::Xbr(_) => Some(Self::Xbr),
            SF::SuperXbr => Some(Self::SuperXbr),
...
impl WgpuScalePipeline {
    pub fn new(device: &wgpu::Device) -> Self {
        ...
        let omniscale_legacy_wgsl =
            include_str!(concat!(env!("OUT_DIR"), "/omniscale_legacy_comp.wgsl"));
        ...
        WgpuScalePipeline {
            ...
            omniscale_legacy: create_compute_pipeline(
                device,
                omniscale_legacy_wgsl,
                "omniscale_legacy",
            ),
    ...
    pub fn encode(
        ...
        let uni_data: [u32; 8] = match filter {
            ...
            | WgpuScaleFilter::Hqx
            | WgpuScaleFilter::Xbr
            | WgpuScaleFilter::Xbrz
            | WgpuScaleFilter::LcdGrid => [src_w, src_h, out_w, out_h, iscale, 0, 0, 0],
        ...
        let uni_bytes: &[u8] =
            unsafe { std::slice::from_raw_parts(uni_data.as_ptr() as *const u8, 32) };
        ...
        if filter == WgpuScaleFilter::ScaleFx {
            ...
            let dispatch_5 = |encoder: &mut wgpu::CommandEncoder,
                ...
                let bg0 = device.create_bind_group(&wgpu::BindGroupDescriptor {
                    ...
                    entries: &[wgpu::BindGroupEntry {
                        binding: 0,
                        resource: px_src.as_entire_binding(),
                    }],
                });
                let bg1 = device.create_bind_group(&wgpu::BindGroupDescriptor {
                    ...
                    entries: &[
                        ...
                        wgpu::BindGroupEntry {
                            binding: 4,
                            resource: px_dst.as_entire_binding(),
                        },
                    ],
                });
                ...
                for pass_idx in 0u32..5 {
                    ...
                    let pass_uni_bytes: &[u8] =
                        unsafe { std::slice::from_raw_parts(pass_uni.as_ptr() as *const u8, 32) };
