use super::{BAD_REGISTERS, CartState, Cartridge, WRONG_MAPPER, ensure, ensure_ram_len};
use crate::clock::Clock;
use std::sync::Arc;

pub struct HuC3 {
    rom: Arc<[u8]>,
    rtc_last_secs: u64,
    clock: Arc<dyn Clock>,
    state: HuC3State,
}

#[derive(Clone, serde::Serialize, serde::Deserialize)]
pub struct HuC3State {
    ram: Vec<u8>,
    rom_bank: usize,
    ram_bank: usize,
    mode: u8, // $0000-$1FFF write selects mode
    #[serde(with = "serde_big_array::BigArray")]
    rtc_mem: [u8; 128], // nybble-addressed RTC/internal memory
    rtc_addr: u8, // current RTC address pointer
    rtc_data_out: u8, // response nybble for reads
    rtc_minutes: u32, // minutes in day (0-1439)
    rtc_days: u32, // day counter
}

impl HuC3 {
    pub(super) fn new(rom: Arc<[u8]>, ram_size: usize, clock: Arc<dyn Clock>) -> Self {
        let rtc_last_secs = clock.now_secs();
        HuC3 {
            rom,
            rtc_last_secs,
            clock,
            state: HuC3State {
                ram: vec![0u8; ram_size.max(0x2000)],
                rom_bank: 1,
                ram_bank: 0,
                mode: 0,
                rtc_mem: [0; 128],
                rtc_addr: 0,
                rtc_data_out: 0,
                rtc_minutes: 0,
                rtc_days: 0,
            },
        }
    }

    fn advance_rtc(&mut self) {
        let now = self.clock.now_secs();
        let elapsed = now.saturating_sub(self.rtc_last_secs);
        let elapsed_mins = elapsed / 60;
        if elapsed_mins == 0 {
            return;
        }
        // Advance the base by whole minutes only, so the partial minute keeps
        // accumulating across frequent polls instead of being discarded.
        self.rtc_last_secs = self.rtc_last_secs.saturating_add(elapsed_mins * 60);
        self.add_minutes(elapsed_mins);
    }

    fn add_minutes(&mut self, mins: u64) {
        let total = (self.state.rtc_minutes as u64).saturating_add(mins);
        let days = u32::try_from(total / 1440).unwrap_or(u32::MAX);
        self.state.rtc_days = self.state.rtc_days.saturating_add(days);
        self.state.rtc_minutes = (total % 1440) as u32;
    }

    fn latch_time_to_mem(&mut self) {
        self.advance_rtc();
        let mins = self.state.rtc_minutes;
        let days = self.state.rtc_days;
        // Minutes at addrs $10-$12 (nybble-addressed, 12-bit BCD-ish)
        self.state.rtc_mem[0x10] = (mins & 0x0F) as u8;
        self.state.rtc_mem[0x11] = ((mins >> 4) & 0x0F) as u8;
        self.state.rtc_mem[0x12] = ((mins >> 8) & 0x0F) as u8;
        // Days at addrs $13-$15
        self.state.rtc_mem[0x13] = (days & 0x0F) as u8;
        self.state.rtc_mem[0x14] = ((days >> 4) & 0x0F) as u8;
        self.state.rtc_mem[0x15] = ((days >> 8) & 0x0F) as u8;
    }

    fn set_time_from_mem(&mut self) {
        self.state.rtc_minutes = (self.state.rtc_mem[0x10] as u32)
            | ((self.state.rtc_mem[0x11] as u32) << 4)
            | ((self.state.rtc_mem[0x12] as u32) << 8);
        self.state.rtc_days = (self.state.rtc_mem[0x13] as u32)
            | ((self.state.rtc_mem[0x14] as u32) << 4)
            | ((self.state.rtc_mem[0x15] as u32) << 8);
        self.rtc_last_secs = self.clock.now_secs();
    }

    fn handle_rtc_cmd(&mut self, cmd_byte: u8) {
        let cmd = (cmd_byte >> 4) & 0x07;
        let arg = cmd_byte & 0x0F;
        match cmd {
            0x1 => {
                // Read + increment address
                let addr = (self.state.rtc_addr as usize) & 0x7F;
                self.state.rtc_data_out = self.state.rtc_mem[addr] & 0x0F;
                self.state.rtc_addr = self.state.rtc_addr.wrapping_add(1) & 0x7F;
            }
            0x3 => {
                // Write + increment address
                let addr = (self.state.rtc_addr as usize) & 0x7F;
                self.state.rtc_mem[addr] = arg & 0x0F;
                self.state.rtc_addr = self.state.rtc_addr.wrapping_add(1) & 0x7F;
            }
            0x4 => {
                // Set address low nybble
                self.state.rtc_addr = (self.state.rtc_addr & 0x70) | (arg & 0x0F);
            }
            0x5 => {
                // Set address high nybble
                self.state.rtc_addr = (self.state.rtc_addr & 0x0F) | ((arg & 0x07) << 4);
            }
            0x6 => {
                // Extended command
                match arg {
                    0x0 => self.latch_time_to_mem(),
                    0x1 => self.set_time_from_mem(),
                    0x2 => self.state.rtc_data_out = 0x01, // status: ready
                    _ => {}
                }
            }
            _ => {}
        }
    }
}

impl Cartridge for HuC3 {
    fn read_rom(&self, addr: u16) -> u8 {
        let idx = match addr {
            0x0000..=0x3FFF => addr as usize,
            0x4000..=0x7FFF => self.state.rom_bank * 0x4000 + (addr as usize - 0x4000),
            _ => return 0xFF,
        };
        self.rom
            .get(idx % self.rom.len().max(1))
            .copied()
            .unwrap_or(0xFF)
    }

    fn write_rom(&mut self, addr: u16, val: u8) {
        match addr {
            0x0000..=0x1FFF => self.state.mode = val,
            0x2000..=0x3FFF => {
                let b = (val & 0x7F) as usize;
                self.state.rom_bank = if b == 0 { 1 } else { b };
            }
            0x4000..=0x5FFF => self.state.ram_bank = (val & 0x03) as usize,
            _ => {}
        }
    }

    fn read_ram(&self, addr: u16) -> u8 {
        match self.state.mode {
            0x00 | 0x0A => {
                let idx = self.state.ram_bank * 0x2000 + (addr as usize - 0xA000);
                self.state
                    .ram
                    .get(idx % self.state.ram.len().max(1))
                    .copied()
                    .unwrap_or(0xFF)
            }
            0x0C => {
                // RTC response read
                0x80 | (self.state.rtc_data_out & 0x0F)
            }
            0x0D => 0x01, // Semaphore: always ready
            0x0E => 0xC0, // IR stub: no signal
            _ => 0xFF,
        }
    }

    fn write_ram(&mut self, addr: u16, val: u8) {
        match self.state.mode {
            0x0A => {
                let idx = self.state.ram_bank * 0x2000 + (addr as usize - 0xA000);
                let len = self.state.ram.len().max(1);
                self.state.ram[idx % len] = val;
            }
            0x0B => {
                // RTC command write
                self.handle_rtc_cmd(val);
            }
            _ => {}
        }
    }

    fn has_battery(&self) -> bool {
        true
    }
    fn ram_data(&self) -> &[u8] {
        &self.state.ram
    }

    fn save_data(&self) -> Vec<u8> {
        let mut data = self.state.ram.clone();
        // Append RTC state: rtc_mem (128 bytes) + minutes (4 bytes LE) + days (4 bytes LE) + timestamp (8 bytes LE)
        data.extend_from_slice(&self.state.rtc_mem);
        data.extend_from_slice(&self.state.rtc_minutes.to_le_bytes());
        data.extend_from_slice(&self.state.rtc_days.to_le_bytes());
        let ts = self.clock.unix_timestamp_secs() as i64;
        data.extend_from_slice(&ts.to_le_bytes());
        data
    }

    fn load_ram(&mut self, data: &[u8]) {
        let ram_len = self.state.ram.len();
        let copy_len = ram_len.min(data.len());
        self.state.ram[..copy_len].copy_from_slice(&data[..copy_len]);

        // Load RTC state
        if data.len() >= ram_len + 128 + 4 + 4 + 8 {
            let rtc_start = ram_len;
            self.state
                .rtc_mem
                .copy_from_slice(&data[rtc_start..rtc_start + 128]);
            let mut buf4 = [0u8; 4];
            buf4.copy_from_slice(&data[rtc_start + 128..rtc_start + 132]);
            self.state.rtc_minutes = u32::from_le_bytes(buf4);
            buf4.copy_from_slice(&data[rtc_start + 132..rtc_start + 136]);
            self.state.rtc_days = u32::from_le_bytes(buf4);
            let mut buf8 = [0u8; 8];
            buf8.copy_from_slice(&data[rtc_start + 136..rtc_start + 144]);
            let saved_ts = u64::try_from(i64::from_le_bytes(buf8)).unwrap_or(0);
            if saved_ts != 0 {
                let elapsed = self.clock.unix_timestamp_secs().saturating_sub(saved_ts);
                self.add_minutes(elapsed / 60);
            }
            self.rtc_last_secs = self.clock.now_secs();
        }
    }
    fn snapshot_state(&self) -> CartState {
        CartState::HuC3(self.state.clone())
    }
    fn validate_state(&self, state: &CartState) -> Result<(), &'static str> {
        let CartState::HuC3(s) = state else {
            return Err(WRONG_MAPPER);
        };
        ensure_ram_len(&s.ram, &self.state.ram)?;
        ensure(s.rom_bank <= 0x7F && s.ram_bank <= 3, BAD_REGISTERS)
    }
    fn restore_state(&mut self, state: &CartState) {
        let CartState::HuC3(s) = state else {
            panic!("{WRONG_MAPPER}");
        };
        self.state.clone_from(s);
        // The RTC counts on from the restored registers.
        self.rtc_last_secs = self.clock.now_secs();
    }
}
