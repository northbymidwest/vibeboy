...
impl Mbc3 {
    ...
    fn advance_rtc(&mut self) {
        ...
        if self.state.rtc_regs[4] & 0x40 != 0 {
            return;
        }
    ...
    fn add_seconds_to_rtc(&mut self, seconds: u64) {
        let mut secs = (self.state.rtc_regs[0] as u64).saturating_add(seconds);
        let mut mins = self.state.rtc_regs[1] as u64 + secs / 60;
        secs %= 60;
        let mut hrs = self.state.rtc_regs[2] as u64 + mins / 60;
        mins %= 60;
        let day_lo = self.state.rtc_regs[3] as u64;
        let day_hi_bit = (self.state.rtc_regs[4] & 0x01) as u64;
        let mut days = day_lo | (day_hi_bit << 8);
        days += hrs / 24;
        hrs %= 24;

        self.state.rtc_regs[0] = secs as u8;
        self.state.rtc_regs[1] = mins as u8;
        self.state.rtc_regs[2] = hrs as u8;
        self.state.rtc_regs[3] = days as u8; // low 8 bits
        ...
        self.state.rtc_regs[4] =
            (self.state.rtc_regs[4] & 0xC0) | ((days >> 8) as u8 & 0x01) | carry;
    }
}
...
impl Cartridge for Mbc3 {
    ...
    fn write_rom(&mut self, addr: u16, val: u8) {
        match addr {
            ...
            0x4000..=0x5FFF => {
                ...
                self.state.ram_bank = v;
            }
            0x6000..=0x7FFF if self.has_rtc => {
                if val == 0x00 {
                    self.state.rtc_latch_ready = true;
                } else if val == 0x01 && self.state.rtc_latch_ready {
                    self.advance_rtc();
                    self.state.rtc_latched = self.state.rtc_regs;
                    self.state.rtc_latch_ready = false;
                } else {
                    self.state.rtc_latch_ready = false;
                }
            }
    ...
    fn read_ram(&self, addr: u16) -> u8 {
        ...
        match self.state.ram_bank {
            ...
            0x08..=0x0C if self.has_rtc => self.state.rtc_latched[self.state.ram_bank - 0x08],
            _ => 0xFF,
        }
    }
    ...
    fn save_data(&self) -> Vec<u8> {
        ...
        if self.has_rtc {
            ...
            for i in 0..5 {
                footer[i * 4] = self.state.rtc_regs[i];
                footer[20 + i * 4] = self.state.rtc_latched[i];
            }
    ...
    fn load_ram(&mut self, data: &[u8]) {
        ...
        self.state.ram[..copy_len].copy_from_slice(&ram_part[..copy_len]);

        let Some(footer) = footer_start.and_then(|start| RtcFooter::parse(&data[start..])) else {
            return;
        };
        self.state.rtc_regs = footer.regs;
        self.state.rtc_latched = footer.latched;
        if footer.saved_ts != 0 && self.state.rtc_regs[4] & 0x40 == 0 {
            let elapsed = self
                .clock
                .unix_timestamp_secs()
    ...
    fn snapshot_state(&self) -> CartState {
        CartState::Mbc3(self.state.clone())
    }
    fn validate_state(&self, state: &CartState) -> Result<(), &'static str> {
        let CartState::Mbc3(s) = state else {
...
mod tests {
    ...
    fn standard_footer_round_trip() {
        ...
        c.state.rtc_regs = [5, 6, 7, 8, 0x01];
        c.state.rtc_latched = [1, 2, 3, 4, 0x00];
        c.state.ram[0] = 0xAB;
        ...
        d.load_ram(&save);
        assert_eq!(d.state.ram[0], 0xAB);
        assert_eq!(d.state.rtc_regs, [5, 6, 7, 8, 0x01]);
        assert_eq!(d.state.rtc_latched, [1, 2, 3, 4, 0x00]);
    }
    ...
    fn footer_with_u32_timestamp_advances_time() {
        ...
        let mut c = cart(0);
        c.load_ram(&data);
        assert_eq!(c.state.rtc_regs[..2], [15, 1]);
    }

    #[test]
    fn day_carry_is_sticky_and_writes_are_masked() {
        let mut c = cart(0);
        c.state.rtc_regs = [0, 0, 0, 0xFF, 0x01];
        c.add_seconds_to_rtc(86_400);
        assert_eq!(c.state.rtc_regs[3..], [0x00, 0x80]);
        c.add_seconds_to_rtc(86_400);
        assert_eq!(c.state.rtc_regs[3..], [0x01, 0x80]);

        c.write_rom(0x0000, 0x0A);
        c.write_rom(0x4000, 0x0C);
        c.write_ram(0xA000, 0xFF);
        assert_eq!(c.state.rtc_regs[4], 0xC1);
        c.write_ram(0xA000, 0x40);
        assert_eq!(c.state.rtc_regs[4], 0x40);
        c.write_rom(0x4000, 0x08);
        c.write_ram(0xA000, 0xFF);
        assert_eq!(c.state.rtc_regs[0], 0x3F);
    }
}
