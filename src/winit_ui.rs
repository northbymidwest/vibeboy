...
impl AudioRing {
    fn new(capacity: usize, stream_rate: u32) -> Self {
...
fn start_audio(ring: Arc<Mutex<AudioRing>>) -> Option<(cpal::Stream, u32)> {
        ...
        match result {
            Ok(stream) => {
                if stream.play().is_ok() {
                    ...
                    return Some((stream, config.sample_rate.0));
                }
            }
            Err(e) => eprintln!("Audio: {}Hz {:?} failed: {e}",
                config.sample_rate.0, config.buffer_size),
        }
    }
